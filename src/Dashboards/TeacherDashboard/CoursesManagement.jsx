  ...
  const toggleCourseModules = async (courseId) => {
    ...
    if (!expandedCourses[courseId] && !modules[courseId]) {
      try {
        ...
        const modulesData = unwrapList(res).filter(
        ...
        setLoadingContent((prev) => ({ ...prev, [`c_${courseId}`]: false }));
      }
    }
  };
  ...
  const toggleModuleLessons = async (moduleId) => {
    ...
    if (!expandedModules[moduleId] && !lessons[moduleId]) {
      try {
        ...
        const lessonsData = unwrapList(res).filter(
        ...
        setLoadingContent((prev) => ({ ...prev, [`m_${moduleId}`]: false }));
      }
    }
  };
  ...
  const stats = useMemo(() => {
    return {
      ...
      totalStudents: myCourses.reduce(
        (acc, c) => acc + (Number(c.students_count) || 0),
        0,
      ),
      pendingRequests: requestHistory.filter((r) => r.status === "pending")
        .length,
    };
                ...
                <tr>
                  <td colSpan="5" className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center justify-center text-gray-500">
                      ...
                      <p className="text-sm">
                        Request a course above and wait for admin approval.
                      </p>
                    </div>
                  </td>
                </tr>
                          ...
                          <div className="bg-gray-50 p-6 border-l-4 border-primary">
                            ...
                            {loadingContent[`c_${course.id}`] ? (
                              <div className="text-center py-4 text-gray-500 flex items-center justify-center gap-2">
                                <FaHourglassHalf className="animate-spin text-primary" />{" "}
                                Loading modules...
                              </div>
                                          ...
                                          <ul className="divide-y divide-gray-100">
                                            {(lessons[module.id] || []).map(
                                              (lesson, idx) => (
                                                <li
                                                  key={lesson.id || idx}
                                                  className="p-3 pl-12 flex justify-between items-center hover:bg-white transition-colors"
                                                >
                                                  <div className="flex items-center gap-3">
                                                    ...
                                                    {lesson.type === "video" ? (
                                                      <FaFileVideo className="text-red-400" />
                                                    ) : (
                  ...
                  <tr key={req.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${getStatusColor(req.status)}`}
                        >
                          {req.status}
                        </span>
                      </div>
